...
public class FacetResponseContributor {
	...
	private String _getCategoryFacetTermDisplayName(
		Locale locale, long categoryId) {
	...
	private String _getFacetTermDisplayName(
		...
		if (StringUtil.equals("category", facet.getName())) {
			...
			return _getSiteFacetTermDisplayName(
				GetterUtil.getLong(term), locale);
		}
		else if (StringUtil.equals("type", facet.getName())) {
			return _getTypeFacetTermDisplayName(locale, term);
		}

		return term;
	}

	private String _getFolderFacetTermDisplayName(
	...
	private String _getSiteFacetTermDisplayName(long groupId, Locale locale) {
	...
	private String _getTypeFacetTermDisplayName(
	...
	private JSONArray _toTermJSONArray(
		...
		if (StringUtil.equals("category", facet.getName()) &&
			StringUtil.equals(
				"assetVocabularyCategoryIds", portalFacet.getFieldName())) {

			return _toVocabularyCategoryTermJSONArray();
		}
		...
		for (int i = 0; i < termCollectors.size(); i++) {
			...
			String displayName = _getFacetTermDisplayName(
	...
	private JSONArray _toVocabularyCategoryTermJSONArray() {
