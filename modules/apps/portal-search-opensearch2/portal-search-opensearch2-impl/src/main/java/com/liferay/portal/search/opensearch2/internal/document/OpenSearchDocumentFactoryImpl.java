...
public class OpenSearchDocumentFactoryImpl
		...
		for (com.liferay.portal.kernel.search.Field field : fields) {
			if (!field.hasChildren()) {
				_addLegacyField(field, translatedFields);
					 ...
					 Validator.isBlank(field.getName())) {

				_addLegacyFields(field.getFields(), translatedFields);
			}
			else {
				_addLegacyNestedField(translatedFields, field);
			}
		}
	}
	...
	private void _addLegacyNestedField(
		...
		translatedFields.put(field.getName(), nestedFieldsMap);

		_addLegacyFields(field.getFields(), nestedFieldsMap);
	}
