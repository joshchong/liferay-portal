...
public class OpenSearchConnectionManagerTest {
	...
	private void _resetAndSetUpMocks() {
		Mockito.reset(
			...
			_operationModeResolver, _remoteOpenSearchConnection1,
			_remoteOpenSearchConnection2, _remoteOpenSearchConnection3,
			_sidecarOpenSearchConnection);

		_setUpDefaultConnection();
		_setUpOpenSearchConfigurationWrapper();
	...
	private void _setUpOpenSearchConfigurationWrapper() {
		Mockito.when(
	...
	private void _setUpSidecarConnection() {
		Mockito.when(
			_sidecarOpenSearchConnection.getConnectionId()
		).thenReturn(
		...
		Mockito.when(
			_sidecarOpenSearchConnection.isActive()
		).thenReturn(
			true
		);
	}
